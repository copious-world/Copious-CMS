...
OAT.Window = function(optObj,type) {
	...
	if ( options.localImage ) {
		if ( !(optObj.hasOwnProperty('imagePath')) ) {  // try to get a system default path...
			var path = window.location.toString();
			path = path.substring(0,path.indexOf("/",8)) + "/hosted/js/oat/images/";
			options.imagePath = path;
		} else {
			var imageUrl = "../js/oat/" + options.imagePath;  // Basic offset to be taken from preferences...
			options.imagePath = imageUrl;
		}
	}
	...
	switch (t) {
		case OAT.WindowData.TYPE_WIN:
			obj = new OAT.MsWin(options);
		break;
		case OAT.WindowData.TYPE_MAC:
			obj = new OAT.MacWin(options);
		break;
		case OAT.WindowData.TYPE_ROUND:
			obj = new OAT.RoundWin(options);
		break;
		case OAT.WindowData.TYPE_RECT:
			obj = new OAT.RectWin(options);
		break;
	}
	if (!obj) { return; }

	obj.resizeTo(options.width,options.height);
	obj.moveTo(options.x,options.y);
...
OAT.WindowParent = function(obj,options) { /* abstract parent for all window implementations */
	...
	obj.caption.style.textAlign = "center";
	...
	obj.resizeTo = function(w,h) {
		...
		if (w && obj) {
			if ( obj.move ) { obj.move.style.width = movew + "px"; }
			if ( obj.div ) { obj.div.style.width = w + "px"; }
			if ( obj.content ) { obj.content.style.width = w + "px"; }
		}
		if (h && obj) {
			try {
				if ( obj.div ) { obj.div.style.height = (h - options.moveHeight) + "px"; }
				if ( obj.content ) { obj.content.style.height = (h - options.statusHeight - options.moveHeight + 3) + "px"; }
			} catch ( e ) {
			} 
		}

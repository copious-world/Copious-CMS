/*
 *  $Id: loader.js,v 1.11 2007/05/18 12:03:38 source Exp $
 *
...
OAT.Dom = { /* DOM common object */
	////
	windowState: { maxDivZ: 1000, lastProp: 0 },
	borders: { left: 1, top: 1, right: -1, bottom: -1 },

	setBorders:function(x,y,r,b) {
		if ( x > 0 ) OAT.Dom.borders.left = x;
		if ( y > 0 ) OAT.Dom.borders.top = y;
		if ( r > 50 ) OAT.Dom.borders.right = r;
		if ( b > 50 ) OAT.Dom.borders.bottom = b;
	},

	withinXBorders:function(x) {
		if ( x > OAT.Dom.borders.left ) {
			if ( OAT.Dom.borders.right == -1 ) return(true);
			if ( x < OAT.Dom.borders.right ) return(true);
		}
		return(false);
	},

	withinYBorders:function(y) {
		if ( y > OAT.Dom.borders.top ) {
			if ( OAT.Dom.borders.bottom == -1 ) return(true);
			if ( y < OAT.Dom.borders.bottom ) return(true);
		}
		return(false);
	},
	...
	moveBy:function(element,dx,dy) {
		...
		*/
		...
		if ( OAT.Dom.withinXBorders(x) ) {
			elm.style.left = x + "px";
		}
		if ( OAT.Dom.withinYBorders(y) ) {
			elm.style.top = y + "px";
		}
	},
	...
	maxZ:function(targetz) {
		if ( targetz <= this.windowState.maxDivZ ) {
			this.windowState.maxDivZ += 2;
		}
		return(this.windowState.maxDivZ);
	}
}
...
OAT.Style = { /* Style helper */
	...
	applyStyleString:function(something,stylestr) {
		var elm = $(something);
		if (!elm) {return;}
		var obj = $strobject(stylestr);
		for (var p in obj) { elm.style[p] = obj[p]; }
	},
...
OAT.Dom.applyStyleString = OAT.Style.applyStyleString; /* backward compatibility */

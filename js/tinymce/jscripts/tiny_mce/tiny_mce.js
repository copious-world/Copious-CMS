...
function TinyMCE_Engine() {
	...
	// NOTE: keyboard layout handler... really badly needed. RAL
	this.keyboardTransformation = false;
...
// Begin code addition here... HERE (RAL-copious-systems)
	isPrintCharKey : function(e) {
		var keyCode = void 0;
		...
		if ( ( keyCode == 8 || keyCode == 46 || keyCode == 13 || keyCode == 17) || e.ctrlKey ) return(false);
		...
		var n = g_printable_chars.length;
		...
		if ( keyCode ) {
			...
			for ( var i = 0; i < n; i++ ) {
				var c = g_printable_chars[i];
				if ( c == CurrentKey ) return(CurrentKey);
			}
		}
		return(false);
	},
	
	changeKey : function(transformer,keyRef) {
		var c = transformer[keyRef];
		return(c);
	},

	setKeyTransformer : function(transformer) {
		this.keyboardTransformation = transformer;
	},
// End code addition here... HERE (RAL-copious-systems)
...
// Begin code addition here... HERE (RAL-copious-systems)
				var keyRef = false;
				if ( ( tinyMCE.keyboardTransformation != false ) && ( (keyRef = tinyMCE.isPrintCharKey(e)) != false ) ){
					//
					var cb = tinyMCE.changeKey(tinyMCE.keyboardTransformation,keyRef);
					tinyMCE.execCommand('mceInsertContent',false,cb);
					tinyMCE.execCommand("mceAddUndoLevel");
					tinyMCE.selectedElement = e.target;
					tinyMCE.triggerNodeChange(false);
					return tinyMCE.cancelEvent(e);

...
OAT.TreeNode = function(li,ul,parent,root) {
	...
	*/
	...
	this.collapse = function() {
		...
		/* check children for selection. if at least one descendant is selected, select this node */
	...
	this.createChildren = function(node_array,checkLeaf) {
	...
	this.findChild = function(testLabel) {
		var n = self.children.length;
		for ( var i = 0; i < n; i++ ) {
			var chck = (self.children[i].getLabel());
			if ( chck == testLabel ) return(true);
		}
		return(false);
	}

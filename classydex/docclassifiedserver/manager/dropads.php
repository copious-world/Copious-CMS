<?php

	// This one is called from 

include '../servicename.php';
try {
	include '../../../database.php';
} catch (Exception $e) {
	echo 'Caught exception: ' .  $e->getMessage();
}


//
// RETURN PREASSEMBLED ROWS...
//

	// create_ad_description
	// returns $addurl
	//
	function delete_ad_descriptions($nowtime) {
		global $db_connection;
		//
		
		////
		$section_maker = "delete from classified_reports ";
		$section_maker .= "where deletion_date <= '$nowtime' ";

		$q_result = @mysqli_query($section_maker) or die (mysqli_error($db_connection));  //
	}
	
	////////////////////////////////////////////////////////////
	
	$nowtime = datetime();
		
	error_reporting(E_ALL);
	delete_ad_descriptions($nowtime);
	
	$reportText = "Deletion for dates pior to $nowtime done.";
	
	echo "$reportText";

?>

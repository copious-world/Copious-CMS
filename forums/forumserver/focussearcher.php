...
//
	...
	function fetch_section($section,$searchterm,$startnumber) {
		...
		$section_q .= " AND ( processed = '1' ) AND ( LOCATE('$searchterm',lister_html_row) > 0 )";
		...
		if ( $counter <= 0 ) {
			...
			if ( $start > $counter ) {
				...
				$classy_q .= " AND ( processed = '1' ) AND ( LOCATE('$searchterm',lister_html_row) > 0 )";
...
// Each row has an href to javascript:display_details(<myid#>);
	...
	$sectionText = fetch_section($sectionID,$searchterm,$startNum);
?>

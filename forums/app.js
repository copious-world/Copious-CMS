...
copyright copious systems 2008
All rights reserved.
...
INFOGROUP.tree = {
	...
	needs:["tree"],
	cb:function() {
		filtered_taxo_pick("Forums",g_forum_taxo_id,"content_type_area","W");
	}
}
...
function forum_topic_check_and_submit() {

	var chck = $('topic_submit_term').value;

	if ( chck.length < 2 ) {
		alert("You need to select a forum to post to.");
		return;
	}
	...
	$('topic_submit_taxo_id').value = g_forum_taxo_id;
	$('topic_submit_term').value = g_currently_selected_forum;
	...
	INFOGROUP.topic1_4W = {
		...
		save_width: 0,
		dontOpen: false,
		app_action: function () { forum_topic_check_and_submit(); },
		cb:function() {
...
//			c.show(100, 100, function(){});
	...
	INFOGROUP.contactForm = {
		...
		app_action: null,
		cb:function() {
			universalWindow(this,"Make Contact","contactDiv","contacterBtn","click");
		}
	}
...
function show_details(topicid) {
	...
	var requrl = forum_locus + "forumserver/details.php?sess=" + g_classydex_session_id + "&topicid=" + topicid;
...
function get_search_entries(term,title_keys,key_keys,search_lb_value,search_ub_value) {
	...
	var requrl = forum_locus + "fetchentries.php?sess=" + g_classydex_session_id + "&forum=" + g_forum_taxo_id + "&term=" + term;
...
var g_currently_selected_forum = "";

function get_forum_topics(term,taxo) {

	setPosterBtnText(term);
	$('forumTitle').innerHTML = term;

	$('topic_submit_term').value = term;
	$('search_forum').innerHTML = term;
	g_currently_selected_forum = term;
	get_search_entries(g_currently_selected_forum,"","","","");
...
function search_current_topics() {
	...
	get_search_entries(g_currently_selected_forum,title_keys,key_keys,search_lb_value,search_ub_value);
				...
				for ( var i = 0; i < n; i++ ) {
					...
					if ( ( accessor == null ) || accessor.hasOwnProperty(name) ) {
						var fetch_fields = (function(term,taxo) {
									return(function(){ get_forum_topics(term,taxo); });
								})(name,g_vocname);

...
$u_group_q = "select count(*) from mail_links where ( UID = '$userid') AND ( grouplabel = '$grouplabel')";
...
if ( $counter <= 0 ) {
	...
	$u_group_q = "select MID from mail_links where ( UID = '$userid') AND ( grouplabel = '$grouplabel')";
	//
	$midlist = db_query_list($u_group_q);
	$qlist = implode(",",$midlist);
...
?>
	...
	if ( g_current_group_object  != null ) {
		g_current_group_object.in_mail_ids = new Array();
		g_current_group_object.mail_in_descr = new Array();
<?php
	for ( $i = 0; $i < $counter; $i++ ) {
?>
		g_current_group_object.in_mail_ids.push(<?php echo $ID_array[$i]; ?>);
		g_current_group_object.mail_in_descr.push(<?php echo $Subject_array[$i]; ?>);
<?php
	}
?>

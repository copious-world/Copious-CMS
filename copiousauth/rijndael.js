...
function mixColumn(state, direction) {
	...
	for ( var j = 0; j < Nb; j++ ) {         // Go through each column...
		...
		//
		for ( var i = 0; i < 4; i++ ) state[i][j] = b[i];        // Place result back into column
		//
	}
}
...
function encrypt(block, expandedKey) {
	...
	//
	for ( i = 1; i < Nr; i++ ) {
		Round(block, expandedKey.slice(j, j + Nb) );
		j += Nb;
	}
...
function hexToByteArray(hexString) {
	//
	var n = hexString.length;
	var byteArray = [];
	
	if ( n % 2 ) {
		alert(hexString);
		return;          // must have even length
	}
...
function rijndaelEncrypt(plaintext, key, mode) {
	...
	var sblock = 0;
	var eblock = bpb;
	for ( var block = 0 ; block < nb; block++ ) {
		
		aBlock = plaintext.slice(sblock,eblock);
		//
		if ( mode == "CBC" ) {
			for ( var i=0; i < bpb; i++ ) aBlock[i] ^= ct[sblock + i];
		}

...
g_expansionlist = [];	/// This is here for historical reasons...
	...
	INFOGROUP.topic_menu2 = {
		...
		wind: null,
		save_width: 0,
		app_action: function() { go_get_profile('user_profile_display'); },
		cb:function() {
			universalWindow(this,"My Profile","topic1_2","mail_menu2","click");
		}
	}
	...
	INFOGROUP.topic_menu10 = {
		...
		app_action: null,
		cb:function() {
			universalWindow(this,"Profile Editor","topic1_10","mail_menu10","click");
		}
	}
	...
	INFOGROUP.topic_menu11 = {
		...
		app_action: null,
		cb:function() {
			universalWindow(this,"Profile Editor - Image Selection","topic1_11","theme_var_IMAGE","click");
		}
	}
...
function go_get_profile(reporter,uid) {
	spanID = reporter;
	var provider = usersown + "fetch_profile.php?sess=" + g_classydex_session_id;
	if ( uid != null ) {
		provider += "&userid=" + uid ;
	}
...
function selectFriend(obj) {
	...
	var id = 0;
	var n = g_current_group_object.members.length;
	for ( var i = 0; i < n; i++ ) {
		if ( g_current_group_object.members[i] == x ) {
			id = g_current_group_object.member_ids[i];
	...
	if ( id > 0 ) {
		go_get_profile("otheruserprofile",id);
	}
...
function prepare_profile_theme_fields() {
	var vdata = "{";
	var sep = '';
	for ( var i = 0; i < n_optional_view_sections; i++ ) {
		...
		if ( !(ctlid_obj) ) {
			...
			btnobj.value = data_from_element(ctlid_obj);
			sep = ',';
		}
	}
	vdata += "}";
...
var g_picture_for_timer_hit = "";

function profile_refeshpicture() {
	$("myprofiletimerhit").src = g_picture_for_timer_hit;
}

function imagerefresh() {

	g_picture_for_timer_hit = $("myprofiletimerhit").src;
//	$("myprofiletimerhit").src = "";

	setTimeout("profile_refeshpicture()",2500);
